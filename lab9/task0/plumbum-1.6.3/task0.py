...
def print_errors(file):
    ...
    """
    print("Error list:")
    awk = local['awk']['{for(i=2;i<=NF;++i)print $i}'][file]
    ...
    sort = local["sort"]
    uniq = local["uniq"]["-c"] # count unique error
    res = awk|sed1|sed2|sed3|sed4|sed5|sed6|grep|sort|uniq
    print(res())
...
def unique_errors(file):
    ...
    grep = local["grep"]['\S'] # remove blanks
    sort = local["sort"]
    uniq = local["uniq"] # count unique error
    wc = local["wc"]["-w"]
    res = awk|sed1|sed2|sed3|sed4|sed5|sed6|grep|sort|uniq|wc
    print(res())
